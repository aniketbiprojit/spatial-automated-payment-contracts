...
contract Executor is AbstractAccessControl {
	...
	function callExecute(
		AbstractInvoice.InvoiceData[] memory invoiceData,
		bytes[] memory signatures
	) external onlyAdmin returns (bool[] memory returnData) {
		...
		for (uint256 index = 0; index < signatures.length; index++) {
			try
				INVOICE_CONTRACT.execute(invoiceData[index], signatures[index])
			{
